<?php 
/* 
Plugin Name: Keyword Snatcher 
Plugin URI: http://impnerd.com/downloads/keyword-snatcher
Description: Displays the percent of each keyword used.
Version: 0.2
Author: impNERD.com
...
if (strpos($_SERVER['REQUEST_URI'], 'post.php') || strpos($_SERVER['REQUEST_URI'], 'post-new.php') || strpos($_SERVER['REQUEST_URI'], 'page-new.php') || strpos($_SERVER['REQUEST_URI'], 'page.php')) {
  ...
  add_action('admin_footer', 'keywordsnatcher');
  
  function keywordsnatcher() {
    echo '<div id="ks1" style="position:absolute; top:150px; left:300px; width:200px; height:300px; background: #fff; border:1px #aaa solid; display: none; overflow: scroll;"></div>';

    echo "<script type=\"text/javascript\">    
      if (keywordToolbar = document.getElementById(\"ed_toolbar\")) {
        var keywordNr, keywordBut, keywordStart, keywordEnd;
            keywordStart = '';
            keywordEnd = '';
            keywordNr = edButtons.length;
            edButtons[keywordNr] = new edButton('keywords','',keywordStart,keywordEnd,'');
            var keywordBut = keywordToolbar.lastChild;
            while (keywordBut.nodeType != 1) {
              keywordBut = keywordBut.previousSibling;
            }
            keywordBut = keywordBut.cloneNode(true);
            keywordToolbar.appendChild(keywordBut);
            keywordBut.value = 'Keywords';
            keywordBut.title = 'Keyword Snatcher';
            keywordBut.onclick = keyword_display; edInsertTag(edCanvas, parseInt(this.title));
            keywordBut.id = 'keywords';
      }
      function close_div() {
        document.getElementById('ks1').style.display = 'none';
      }
      function keyword_display() {
        document.getElementById('ks1').style.display = 'inline';
        var str = document.forms['post'].elements['content'].value;
       ...
       larr = arr.length;
        ...
        function countAppearance(arr) {
          var myPerc = new Object;

          for(var i=0; i<larr; i++) {
            var aSame = arr[i];
            if (stopWords.indexOf(aSame) != -1) {
              arr.splice(i, 1);
            }
            else {
              if(myPerc.hasOwnProperty(aSame)) {
                myPerc[aSame]++;
              } else {
                myPerc[aSame] = 1;
              }
          ...
          for(var j in myPerc) {
            if (j != '' && j != 'undefined') {
            str += '' + j + ': ' + Math.round((myPerc[j] / larr) * 1000) / 10 + '%<br />';
            }
          }
        
          document.getElementById('ks1').innerHTML = '<strong>Keywords:</strong> <a onclick=\"close_div()\">[close]</a><br />' + str;
